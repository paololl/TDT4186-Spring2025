...
int quanta[NQUEUES] = {1, 2, 4}; // Time quantum for each queue

Queue pQueues[NQUEUES]; // Queues for MLFQ

int nextpid = 1;
struct spinlock pid_lock;
struct spinlock runq_lock; // Lock for run queue
...
//Helper functions for MLFQ implementation
void initQueue();
...
void mlfq_scheduler(void); //MLFQ scheduler implementation
void opt_scheduler(void); // Optimized scheduler implementation
#define SCHEDC 3
static SchedImpl available_schedulers[SCHEDC] = {
    {"Round Robin", &rr_scheduler, 1},
    {"MLFQ", &mlfq_scheduler, 2}, // Added MLFQ
    {"Optimized", &opt_scheduler, 3} // Added Optimized
};
...
void userinit(void)
{
    ...
    // Set up trapframe for the first user process.
...
// Initialize the queues for MLFQ
void initQueue(){
